...
var (
	BACCOUNT_SERVE = &BAccountServe{
		PortableTemplate: PortableTemplate{Impl: &BAccountPortableImpl{}},
	}
	a0_mbase   = mongoc.NewMongoColl("account_0", 20*time.Second)
	a1_mbase   = mongoc.NewMongoColl("account_1", 20*time.Second)
	a100_mbase = mongoc.NewMongoColl("account_100", 20*time.Second)
	an1_mbase  = mongoc.NewMongoColl("account_n1", 20*time.Second)

	at_mbase = mongoc.NewMongoFIFOColl("account_tag", 20*time.Second, 100)
)

type BAccountServe struct {
...
func (b *BAccountServe) ImportTag(tag string, interval int64) error {
...
func (b *BAccountServe) SaveTagNum(tag string, num int64) error {
	return at_mbase.UpdateOne(bson.M{"Tag": tag}, bson.M{"$set": bson.M{"Num": num}})
}

func (b *BAccountServe) IncBindingNum(tag string) error {
	return at_mbase.UpdateOne(bson.M{"Tag": tag}, bson.M{"$inc": bson.M{"BindingNum": 1}})
}

func (b *BAccountServe) ImportFromCSV(file multipart.File) int64 {
	return b.LineImport(a0_mbase, file)
}

func (b *BAccountPortableImpl) ImportParse(line string) (interface{}, error) {
	ep := strings.Split(line, ",")
	if dType == entities.SEC_TYPE_DIRECT {
		if len(ep) == 2 {
			return entities.AppleId{Email: ep[0], Password: ep[1], Tag: BAPPLEID_SERVE.CurrentServedTag, SecType: dType, DispNum: 0, CreateAt: time.Now()}, nil
		}
	}
	if dType == entities.SEC_TYPE_APICODE {
		if len(ep) == 3 {
			return entities.AppleId{Email: ep[0], Password: ep[1], Tag: BAPPLEID_SERVE.CurrentServedTag, SecAPI: ep[2], SecType: dType, DispNum: 0, CreateAt: time.Now()}, nil
		}
	}
	if dType == entities.SEC_TYPE_FIXCODE {
		if len(ep) == 3 {
			return entities.AppleId{Email: ep[0], Password: ep[1], Tag: BAPPLEID_SERVE.CurrentServedTag, SecCode: ep[2], SecType: dType, DispNum: 0, CreateAt: time.Now()}, nil
		}
	}
	return entities.AppleId{}, fmt.Errorf("Field error")
}

func (p *BAppleIdPortableImpl) ExportWrite(writer *csv.Writer, entity interface{}) error {
	appleId := entity.(entities.AppleId)
	if appleId.IsSecEmpty() || appleId.SecType == entities.SEC_TYPE_DIRECT {
		err = writer.Write([]string{appleId.Email, appleId.Password})
	} else if appleId.SecType == entities.SEC_TYPE_APICODE {
		err = writer.Write([]string{appleId.Email, appleId.Password, appleId.SecAPI})
	} else if appleId.SecType == entities.SEC_TYPE_FIXCODE {
		err = writer.Write([]string{appleId.Email, appleId.Password, appleId.SecCode})
	}
}

func (s *BAccountServe) FetchAppleId(sn string) (entities.AppleId, error) {
...
func (s *BAccountServe) NoBarrier() (bool, error) {
	es, err := a0_mbase.EstimatedSize()
	return es > 0, err
}

func (s *BAccountServe) Export(writer *csv.Writer, status int64) {
...
func (s *BAccountServe) getMCollByIndex(status int64) (*mongoc.MongoColl, error) {
	switch status {
...
func (s *BAccountServe) Truncate(collIndexs ...int64) error {
...
func (s *BAccountServe) SizeOfCollIndex(collIndex int64) (int64, error) {
...
func (s *BAccountServe) DisabledStatus(email string) error {
...
func (s *BAccountServe) AppleIdBindSucc(email string) entities.AppleId {
...
func (s *BAccountServe) FindByEmail(email string) (entities.AppleId, error) {
	id := entities.AppleId{}
...
func (s *BAccountServe) RecyleForFailed() (int64, error) {
...
func (s *BAccountServe) AggregateForColl(coll *mongoc.MongoColl) ([]tagAggre, error) {
...
func (s *BAccountServe) ListTag() ([]entities.AppleIdTag, error) {
...
func (s *BAccountServe) FindIntervalByTag(tag string) int64 {
...
func (s *BAccountServe) TagIntervalByEmail(email string) (string, int64) {
	appleId := entities.AppleId{}
...
func (s *BAccountServe) SaveTagInterval(tag string, interval int64) error {
...
func (s *BAccountServe) TagDel(tag string) error {
